...
- authentification locale avec abstraction prévue pour un futur passage à Keycloak via OpenID Connect
...
Par défaut, l’authentification OIDC est désactivée. Si vous souhaitez utiliser Keycloak comme broker entre l’application et LDAP, ajoutez ou modifiez `OIDC_ENABLED=true` dans votre fichier `.env` et configurez les variables `OIDC_*` associées. Les anciens noms `KEYCLOAK_*` restent acceptés pour compatibilité.

Variables utiles :

- `OIDC_METADATA_URL` : URL de découverte OpenID Connect de votre IAM
- `OIDC_CLIENT_ID` : client OAuth/OIDC de l’application
- `OIDC_CLIENT_SECRET` : secret du client
- `OIDC_REDIRECT_URI` : URL de callback exacte enregistrée côté IAM
- `OIDC_REQUIRED_GROUPS` : groupes optionnels requis, séparés par des virgules
- `OIDC_PROVIDER_NAME` : libellé affiché dans l’interface, par défaut `Keycloak`
